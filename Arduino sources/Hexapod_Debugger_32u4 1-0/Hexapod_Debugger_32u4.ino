...
void loop()
{
  ...
  if(Serial.available())
  {
    ...
    if(message.equalsIgnoreCase("help"))
    {
      ...
      Serial.println(F("  autoprint slave"));
      Serial.println(F("  autocalibrate slave"));
    ...
    else if(message.equalsIgnoreCase("band pass test"))
    {
      ...
      Serial.print(F(", LEGBOARD : ")); Serial.println((int)slaveCounter);
    ...
    {
      Serial.println(F(">> reset scs15 positions .."));
      SCS15.enableTorque(BROADCAST_ID, 1);
    ...
    else if(message.equalsIgnoreCase("set slave id"))
    {
      ...
      if(oldid > 0 && oldid <= BROADCAST_ID && newid > 0 && newid < BROADCAST_ID)
      {
        int error = LEGBOARD.setPermanentID(oldid, newid);
        if(error)
        {
          Serial.print(">> error : ");
          Serial.println(error);
    ...
    else if(message.equalsIgnoreCase("autoprint slave"))
    {
      ...
      while(!Serial.available())
      {
        if(!LEGBOARD.debug(pingId))
          Serial.println(F("slave not resonding"));
        Serial.println(F(">> send any message to stop autoprint"));
        delay(1000);
      }
      Serial.println(F(">> end"));
    }
    else if(message.equalsIgnoreCase("autocalibrate slave"))
    {
      int pingId = askUserFor("id");
      if(!LEGBOARD.autocalibrate(pingId))
        Serial.println(F("slave not resonding"));
      else Serial.println(F(">> success"));
    }
    else if(message.equalsIgnoreCase("set slave color"))
    {
      ...
      if(id > 0 && id <= BROADCAST_ID)
      {
        ...
        if(LEGBOARD.setRegister(id, REG_LED_RED, 1, &color[0])!= 6) Serial.println("error red");
        else if(LEGBOARD.setRegister(id, REG_LED_GREEN, 1, &color[1])!= 6) Serial.println("error green");
        else if(LEGBOARD.setRegister(id, REG_LED_BLUE, 1, &color[2])!= 6) Serial.println("error blue");
        else Serial.println(F(">> success"));
      }
    }
    else if(message.equalsIgnoreCase("set slave blink mode"))
    {
      ...
      if(id > 0 && id <= BROADCAST_ID)
      {
        ...
        if(LEGBOARD.setRegister(id, REG_STATE, 1, &mode)!= 6) Serial.println("error changing mode");
        else Serial.println(F(">> success"));
      }
    }
    else if(message.equalsIgnoreCase("set slave thresholds"))
    {
      ...
      if(id > 0 && id <= BROADCAST_ID)
      {
        ...
        for(;;)
        {
          ...
          if(LEGBOARD.setRegister(id, REG_STATE, 1, &locker)!= 6)
          {
            Serial.println(F("error while unlocking EEPROM"));
            break;
          }
          if(LEGBOARD.setRegister(id, REG_SIZE + EEPROM_DISTANCE_THSD_H, 2, (uint8_t*)&distance) != 6)
          {
