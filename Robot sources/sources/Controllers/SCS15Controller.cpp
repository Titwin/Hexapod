...
int SCS15Controller::scan(uint8_t* idSize, uint8_t* idList)
{
    int ackcount = 0;
    if(idSize) *idSize = 0;
...
bool SCS15Controller::debug(const uint8_t& ID, uint8_t* response)
{
...
void SCS15Controller::syncSetRegisterWord(const uint8_t& IDN, uint8_t* ID, const uint8_t& regStart, uint16_t* value)
{
