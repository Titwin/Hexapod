...
void Hexapod::animateWalk(float elapseTime, MyVector3f translationSpeed, MyVector3f rotationSpeed)
{
    ...
    for(int i=0;i<6;i++)
    {
        ...
        targetAnglesArray[i] = inverseKModule->getAnglesFromExtremityAndIncidence(i,legSendPosition[i],legsIncidence[i],motorAnglesArray[i]);
    }
}
...
void Hexapod::updateLegPositionFromMotorsAngles()
{
    ...
    if(false/*arduino->readMessage(tmpBuff,54,34)*/)
    {
        for(int i=0;i<6;i++)
        {
...
bool Hexapod::tryConnectingToArduino()
{
    ...
    std::cout<<" fail\nUnable to connect to Arduino"<<std::endl;
    return false;
}
